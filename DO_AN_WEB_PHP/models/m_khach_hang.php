<?php
    ...
    class M_khach_hang extends database {
        ...
        function themKhachHang($cus_name,$cus_email,$cus_phone,$cus_address,$cus_note) {
            $query = "INSERT INTO `customer`( `cus_name`, `cus_email`, `cus_phone`, `cus_address`, `cus_note`) VALUES (?,?,?,?,?)";
            $this->setQuery($query);
            $result = $this->execute(array($cus_name,$cus_email,$cus_phone,$cus_address,$cus_note));
            if($result)
        ...
        function themChiTietHoaDon($b_id,$pr_id,$pb_count,$pb_cprice,$pr_name,$pr_image) {
            $query = "INSERT INTO `pbill`(`b_id`, `pr_id`, `pb_count`, `pb_cprice`,`pr_name`,`pr_image`) VALUES (?,?,?,?,?,?)";
            $this->setQuery($query);
            return $this->execute(array($b_id,$pr_id,$pb_count,$pb_cprice,$pr_name,$pr_image));
        }
